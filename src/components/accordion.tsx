...
>(function ({ className, children, onClick, ...props }, ref) {
  ...
  return (
    ...
    >
      ...
      <ChevronDownIcon
        role="presentation"
        className={cn(
          "h-auto w-4 fill-slate-900/70 transition-transform duration-300",
...
>(function ({ className, ...props }, _ref) {
      ...
      if (divStyles.getPropertyValue("--mh") === "") {
        ...
        setTimeout(function () {
          ...
          outerDiv.style.setProperty(
            "--mh",
            clonedDiv.clientHeight.toString() + "px",
          );
          clonedDiv.remove();
        });
      }
  ...
  return (
    <div
      ...
      aria-selected={accordionType === "single" ? open === value : undefined}
      className={cn(
        "overflow-y-clip text-sm text-slate-900/70 transition-[max-height,padding] duration-300",
        className,
        open !== value && "max-h-0 py-0",
        open === value && "max-h-[var(--mh)]",
      )}
      {...props}
    />
  );
