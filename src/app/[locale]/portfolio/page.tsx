          ...
          <CarouselContent>
            {
              websites.map(({ namespace, src }, index) => (
                <CarouselItem className="basis-1/3" key={index + 1}>
                  <Link href={"/portfolio/" + (index + 1).toString()} className="flex flex-col">
                    <Image src={src} alt={t("websites." + namespace + ".name")} width={2880} height={2048} sizes="40vw" className="rounded-xl mb-2 border-[6px] border-transparent bg-[linear-gradient(to_bottom_right,#0EA5E9,#34D399)] bg-[length:calc(100%+12px)_calc(100%+12px)] bg-[position:-6px_-6px] bg-no-repeat object-cover shadow lg:basis-1/2" />
