...
export async function generateMetadata({ params }: { params: Params }) {
  return await metadata({
    params,
    namespace: "portfolio.websites.sis",
  });
}

export default async function SistersInSTEM({ params }: { params: Params }) {
  ...
  return (
    <main className="flex flex-col gap-4 p-4">
      <header className="bg-linear-to-br flex flex-col gap-4 rounded-xl from-sky-500 to-emerald-400 p-4 text-slate-50 shadow-sm lg:basis-1/2 lg:p-6">
        <h1 className="text-4xl">{t("websites.sis.name")}</h1>
        <p className="text-xl">{t("websites.sis.description")}</p>
      </header>
      <main className="flex flex-col gap-4 lg:flex-row">
        <main className="flex flex-col gap-4 lg:basis-1/2">
          <section className="flex flex-col gap-2">
            ...
            <p className="text-xl">{t("websites.sis.problem")}</p>
          </section>
          <section className="flex flex-col gap-2">
            <h2 className="text-3xl">{t("solution")}</h2>
            <p className="text-xl">{t("websites.sis.solution")}</p>
          </section>
